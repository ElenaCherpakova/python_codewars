...
def get_mean(arr,x,y):
    if 1 < x <= len(arr) and 1 < y <= len(arr):
        sum_firs = sum(arr[:x]) / x
        sum_last = sum(arr[-y:]) / y
        return (sum_firs + sum_last) / 2
    else:
        return -1

print(get_mean([1,2,3,4],2,3)) #2.5
print(get_mean([1,2,3,4],1,2)) #-1
print(get_mean([1,2,3,4],2,8)) #-1
